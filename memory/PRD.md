...
- **Uncategorized header now collapsible** — On the home page (both list and grid views), the "Uncategorized" section header was a pure display component and tapping it did nothing — the tiles inside stayed expanded. `CategoryHeader.jsx` now accepts optional `onToggle`/`isOpen` props: when provided it renders as a `<button>` with a rotating ChevronDown that folds the tiles into the header, matching the behaviour of normal named categories (`CategoryGroup`). State is owned in `NotesApp.jsx` (`uncategorizedOpen`) and persisted per-device to `localStorage["ir_uncategorized_open"]`. Grid + list branches both hide the children via a CSS `hidden` class while keeping the react-beautiful-dnd Droppable/placeholder in the DOM so drag-into-uncategorized still works. Service worker cache bumped `iron-rabbit-v44` → `iron-rabbit-v45`. Files: `frontend/src/notes/CategoryHeader.jsx`, `frontend/src/NotesApp.jsx`, `frontend/public/service-worker.js`.
- **Home page "white strip" fix (mobile list view)** — On the home page the app-container previously fell back to `bg-gray-50` (light) or `bg-[#020617]` (dark) below the last list item, producing a visible white/dark strip that didn't match `<main>`'s composited brightness. Added a small `composeHomeAppBg()` helper in `NotesApp.jsx` that computes the exact final colour `<main>` paints (base × underlay × main-black alpha) and applies it as an inline `backgroundColor` on the app-container div. Zero touches to the locked underlay/`<main>` block. Files: `frontend/src/NotesApp.jsx`.
