...
- **Alert Dismiss / Snooze** (2026-02-07) — Each row in the Recovery Alerts Log panel now has a "7d" snooze button (Clock icon) and a "×" dismiss button. Backend `PATCH /api/admin/recovery/alerts/{week_start}` accepts `{action: 'dismiss'|'snooze'|'restore', days?}` (snooze days clamped 1-30, default 7). Default list query hides rows with `dismissed_at` set OR `snooze_until > now` (both `$exists:false` and null count as visible so legacy rows aren't stranded). `?include_dismissed=1` shows everything for audit. Optimistic UI: dismissed row disappears instantly. Verified: dismiss + snooze + restore + 400 on bad action + 404 on unknown week — all correct. Testids: `alerts-log-snooze-N`, `alerts-log-dismiss-N`.
