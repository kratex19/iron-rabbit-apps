...
- **Thank-You Email Preview** (2026-02-07) — `POST /api/community/tips/{id}/promote?preview=1` (admin-only) renders the thank-you email HTML with `text/html` content-type WITHOUT mutating the tip status or sending anything. Refactored `_send_thank_you` to delegate HTML generation to a pure `_render_thank_you_html(tip)` helper so preview = exactly what will fly. Admin dashboard gets an "👁 Preview email" button next to Promote (only on pending tips with opt-in + email); click opens a sandboxed-iframe modal (`thankyou-preview-modal`) showing the email + subject line.
- **Screenshot Regen Cron** (2026-02-07) — New `scripts/regen_play_gallery.py` (async entry that imports capture + overlay as modules — no subprocess overhead). APScheduler weekly job runs it every **Sunday 07:00 UTC**, guarded by the new `SCREENSHOT_CRON_ENABLED` env var (default off — preview env stays quiet; flip to `1`/`true`/`yes` in production). Admin endpoints `POST /api/admin/screenshots/regen` (single-flight via `asyncio.Lock`, 409 if already running, returns 200 immediately) and `GET /api/admin/screenshots/status` (last 5 runs + `running` flag). Run history persisted in `screenshot_runs` collection for audit.
