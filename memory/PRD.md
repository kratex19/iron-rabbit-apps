...
- **Audit Trail View** (2026-02-07) — `RecoveryAlertsLog.jsx` gained an "Include dismissed" toggle (Eye/EyeOff icon) that flips the fetch to `?limit=20&include_dismissed=1`. When the active list is empty, the panel collapses to a minimalist "Show recovery-alert audit trail" pill (data-testid `alerts-log-audit-toggle` in both states) so the toggle stays reachable without cluttering the dashboard. Full view renders per-row status badges: green `AUTO-DISMISSED`, slate `Dismissed`, indigo `Snoozed` — plus a `Restore` action that clears both fields. Rows in non-active state get a 70% opacity for visual demotion.
