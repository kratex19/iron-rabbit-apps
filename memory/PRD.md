...
- **Community Digest Email (Resend, admin-only)** — New `POST /api/community/digest/send` endpoint renders a table-based HTML digest of pending + recently-promoted tips and dispatches via Resend to `ADMIN_DIGEST_EMAIL`. Supports `?dry_run=1` for previewing without sending. Gracefully returns `{ok:false, reason:"RESEND_API_KEY not configured on server"}` when the key is missing — no exception, no user-visible crash. Wired to a "Send digest" button in the Community Dashboard header. Resend key still pending from user; `SENDER_EMAIL=digest@ironrabbitapps.com` (needs DNS records to be added on ironrabbitapps.com).
