...
- **Home page "white strip" fix (mobile list view)** — On the home page the app-container previously fell back to `bg-gray-50` (light) or `bg-[#020617]` (dark) below the last list item, producing a visible white/dark strip that didn't match `<main>`'s composited brightness. Added a small `composeHomeAppBg()` helper in `NotesApp.jsx` that computes the exact final colour `<main>` paints (base × underlay × main-black alpha) and applies it as an inline `backgroundColor` on the app-container div. Zero touches to the locked underlay/`<main>` block — the fix lives entirely on the outer wrapper. Now the region below the lowest note visually continues the same shade as the last row. Auto-tracks the text-brightness `bg` slider. Service worker cache bumped `iron-rabbit-v43` → `iron-rabbit-v44`. Files: `frontend/src/NotesApp.jsx`, `frontend/public/service-worker.js`.
