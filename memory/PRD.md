...
- **Admin Recovery Funnel Panel** (2026-02-07) — New `RecoveryFunnel.jsx` component mounted on `/admin/community` right below the featured-tip analytics chart. Big-number card shows `magic_link_share` (e.g. "70%") — the single metric that answers "is the deep link worth its URL length?". Supporting stats: Emails minted, Magic-link opens, Manual opens, Verified (with conversion %), Wrong codes, Window. Window picker (7d/30d/90d) hits `/api/community/recovery/analytics?days=N`. Amber tint on Wrong codes when failure rate > 50%. Data-testids: `recovery-funnel-panel`, `funnel-magic-share`, `funnel-window-7|30|90`, `funnel-email-sent`, `funnel-magic-opened`, `funnel-manual-opened`, `funnel-verify-success`, `funnel-verify-failed`, `funnel-refresh`.
