...
- **Featured Tip Rotation (home-screen)** — New public endpoint `GET /api/community/featured` deterministically picks ONE promoted tip per UTC date (rotates daily). New `FeaturedTipStrip.jsx` mounts above the notes list showing "COMMUNITY · TIP OF THE DAY" with heading + body + "Open guide" button + dismiss X. Dismiss persists for the day via `localStorage['irr.featured_tip_hidden_date']`. Silent when no promoted tips exist. Never breaks first-run UX.
- **Digest Schedule (weekly cron + unsubscribe)** — APScheduler in-process job registered at FastAPI startup, fires every Monday 09:00 UTC. `_send_digest_now(force_when_empty=False)` skips silently when zero pending tips (no inbox spam). New admin endpoints: `GET /api/community/digest/status` (returns `{enabled, last_sent_at, sender_email, recipient_email, resend_key_configured, scheduler_next_run}`), `POST /api/community/digest/toggle {enabled:bool}`, public `GET /api/community/digest/unsubscribe?token=...` with themed HTML confirmation ("You are unsubscribed" or "Link expired"). Per-install unsubscribe token auto-generated in `app_config` collection. Community Dashboard header shows a live status chip with next-run, last-sent, "RESEND_API_KEY missing" warning, and Turn ON/OFF button. Unsubscribing sets enabled=false → future cron sends and manual sends are gated → admin can re-enable from the dashboard.
- **Import from Text (LLM bulk import)** — New `POST /api/community/tips/parse` endpoint uses the Emergent LLM key (claude-sonnet-4-6) to structure pasted text into clean `{heading, body}` cards (max 20). New `PasteTipsDialog` component lives in two modes: `admin` (Bulk paste button in Community Dashboard → parse → per-card include toggle + inline edit + delete → Save as pending OR Promote all in one click) and `quickguide` (a "Paste multiple tips" card at the end of the Quick Guide scroll strip → parse → add all as user cards on the current guide). Graceful fallbacks for empty text, 8000-char cap, and messy model output.
- **Community Digest Email (Resend, admin-only)** — `POST /api/community/digest/send` endpoint renders a table-based HTML digest of pending + recently-promoted tips and dispatches via Resend. Supports `?dry_run=1`. Gracefully returns `{ok:false, reason:"RESEND_API_KEY not configured on server"}` when the key is missing. Wired to a "Send digest" button in the Community Dashboard header.
- **Community Admin Dashboard** — Private `/admin/community` route with `AdminGate` (token-based via `ADMIN_TOKEN` env). Lists submitted community tips grouped by status (Pending / Promoted / Rejected) with live counts, filter, refresh, and per-tip Promote / Reject / Restore / Delete actions.
- **QR Scan-Back** — Iron Rabbit tip QR payloads now include a machine-readable `[IRTIP1:<url-safe-b64-json>]` marker. `BarcodeScannerModal` detects the marker and short-circuits to `QuickGuideProvider.pendingImport` → "Import this tip?" preview inside the guide modal.
- **Promoted community tips surface in Quick Guide** — Provider fetches `/api/community/promoted` on mount and renders matching tips as read-only emerald-badged "Community" cards.
