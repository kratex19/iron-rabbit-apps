...
- **Wall "New" Ribbon** (2026-02-07) — Cards with `latest_promoted_at` within the last 7 days now render a small amber "NEW" badge. `You` badge takes precedence (a returning owner's own card never gets duplicate-badged). Renders in both Top and Recent sort modes. Data-testid: `wall-new-badge-<nickname>`. See `isFreshlyPromoted()` in `ContributorWall.jsx`.
