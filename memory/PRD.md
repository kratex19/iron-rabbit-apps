...
- **Repair #6 · Touch Cross-Category Drag = MOVE (v168)** — Fixed touch cross-category drops copying-with-fresh-uuid instead of moving-in-place. Root cause: `modifier` was derived only from Cmd/Ctrl, which a TouchEvent cannot carry, so touch dispatched `mode='copy'`. Fix: `handleDragStart` also detects `activatorEvent.touches` (TouchSensor) or `pointerType === "touch"` (PointerSensor) and promotes `modifier=true` for those drags. Downstream unchanged. Desktop semantics (Cmd/Ctrl = MOVE, no modifier = COPY) preserved verbatim. Verified iteration_91 (4/4 primary contracts PASS). Reported (not fixed) latent Repair #5 stale-srcRect on mobile wobble — deferred to future Repair #7.
- **Repair #7 · Stale Source-Rect During Touch Drag (v169)** — Fixed Repair #5's boundary predicate misfiring under mid-drag DOM reflow on mobile grouped layouts (~214px rect drift observed). Root cause: `getBoundingClientRect()` at drop time can be stale. Fix: `srcRectRef` snapshots the source pack's rect at drag-start (layout stable), and `handleDragEnd` consumes the snapshot with live fallback for fail-closed safety. Repair #5 predicate byte-identical, just fresher input. Verified iteration_92 (8/8 PASS including the exact iter_91 wobble reproducer, TEST 8A regression, reload persistence, desktop copy/move unchanged, same-pack reorder, consecutive-drag leak check, cancel path).
- **Repair #8 · Serpentine Deep-Hierarchy Visual Direction (v170)** — Implements Code Rabbit's four-level reversal spec. Every 4 levels the indent direction flips (L1-L4 forward, L5-L8 reverse, L9-L12 forward, L13-L16 reverse, indefinite pattern). Presentation-only: `NestedSubGroup.jsx` computes `hierarchyLevel = depth + 1` and `inReverseBand = Math.floor((L-1)/4) % 2 === 1`, then swaps `ml-4 mr-0` ↔ `ml-0 mr-4` on the wrapper and toggles `flex-row-reverse` on the header. Two new observability data attributes (`data-hierarchy-level`, `data-serpentine-band`). Zero data-model changes; Repair #1 invariant preserved; TEST 8 + TEST 9 + TEST 10 all still pass. Verified iteration_96 (V1-V10 PASS on applicable sub-tests; V4 grid drilldown marked N/A due to pre-existing grid+group_by_category flattening).
