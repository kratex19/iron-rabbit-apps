...
- **Note expand transparency polish** (2026-02-07) — Both note-expand surfaces now use glass-morphism instead of opaque dark backdrops, so the app wallpaper bleeds through:
  - `FullScreenNote.jsx` line 82: `bg-[#0B1221]` → `bg-[#0B1221]/75 backdrop-blur-2xl` (dark mode only).
  - `constants.js` `getNoteColorStyle()`: dark-mode base changed from solid `#0B1221` → `rgba(11, 18, 33, 0.55)`, so the tint gradient overlays a **translucent** dark base. Affects both the inline accordion expand (list view) and the FullScreenNote background layer. Light mode untouched.
  - Text colors unchanged (`text-slate-100` / `text-white` in dark mode, `text-gray-900` in light mode). Verified via Playwright — accordion expand shows purple test note with white body copy on translucent tinted bg; FullScreenNote shows title + body in white on visibly-blurred dark glass with purple accent border.
- **Desktop website screenshots** (2026-02-07) — New `scripts/capture_desktop_screenshots.py` (Playwright, 1920×1080) captures 9 landscape shots. Combined with 4 pre-existing landscape shots → **13 total** in `/app/frontend/public/screenshots/desktop/` + `screenshots/`. Bundled zip at `/app/frontend/public/downloads/ironrabbit-desktop-screenshots.zip` (1.2 MB) — served publicly via preview URL. Companion `scripts/email_desktop_screenshots.py` (Resend attachment sender) ready once `RESEND_API_KEY` is populated.
- **Notifications spacing polish** (2026-02-07) — `SettingsModal.jsx`: `gap-2 mb-2` → `gap-3 mb-3` + `ml-auto` on Enable/Send-test buttons.
- **`/app/ENV_REFERENCE.md`** (2026-02-07) — Single-page catalog of every launch-mode env var with purpose, examples, where-used, and a copy-paste `.env` skeleton.
- **Platform-support audit answered** (2026-02-07) — Android ✅ code-ready, iOS ❌ (no `ios/` folder, no Apple assets), PWA ✅, Offline-First ✅, Capacitor ✅, Play Store ✅ ready, App Store ❌ (13-step checklist provided).
