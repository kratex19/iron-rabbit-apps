...
## Backlog / Roadmap
- **P1**: Repair #7 — snapshot source-pack `srcRect` at `handleDragStart` (or use `IntersectionObserver`-driven live rect map) instead of at drop time, to fix Repair #5's stale-bounding-rect corner case exposed by mobile wobble on two-pack layouts (surfaced during Repair #6 verification, iteration_91).
