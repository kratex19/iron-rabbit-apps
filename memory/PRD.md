...
- **Desktop website screenshots** (2026-02-07) — New `scripts/capture_desktop_screenshots.py` (Playwright, 1920×1080) captures 9 landscape shots: Home, Note fullscreen, Quick Guide, Settings, Wall Top, Wall Recent, Share Card, Recovery Dialog, Magic-Link Landing. Combined with the 4 pre-existing landscape hero shots (kid-mode, pantry-nutriscore, meal-plan, backup-restore) → **13 total desktop-sized screenshots** in `/app/frontend/public/screenshots/desktop/` and `/app/frontend/public/screenshots/`. Bundled zip available at `/app/frontend/public/downloads/ironrabbit-desktop-screenshots.zip` (~1.2 MB). Zip is served publicly at `${REACT_APP_BACKEND_URL}/downloads/ironrabbit-desktop-screenshots.zip` for direct download. Playwright script uses `add_init_script` to suppress QG auto-open and `about:blank` reset between shots to avoid `ERR_ABORTED` on same-URL nav.
- **`scripts/email_desktop_screenshots.py`** (2026-02-07) — Resend-backed email sender that ships all desktop shots as attachments (base64 encoded). Ready to run once `RESEND_API_KEY` is populated in `backend/.env` (currently empty — email send blocked).
- **Notifications spacing polish** (2026-02-07) — In Settings → Notifications panel, the header row now uses `gap-3 mb-3` (was `gap-2 mb-2`) and the *Enable* / *Send test* buttons carry `ml-auto` so they align right with breathing room after the status badge. Zero functional changes. File: `frontend/src/notes/SettingsModal.jsx`.
- **`/app/ENV_REFERENCE.md`** (2026-02-07) — Single-page catalog of every launch-mode env var: core (`MONGO_URL`, `DB_NAME`, `CORS_ORIGINS`), frontend (`REACT_APP_BACKEND_URL`), admin (`ADMIN_TOKEN`, `ADMIN_DIGEST_EMAIL`), LLM (`EMERGENT_LLM_KEY`), email (`RESEND_API_KEY`, `SENDER_EMAIL`, `PUBLIC_APP_URL`), Slack (`SLACK_WEBHOOK_URL`, `SLACK_SIGNING_SECRET`), auto-dismiss (`AUTO_DISMISS_SHARE_THRESHOLD`), and cron (`SCREENSHOT_CRON_ENABLED`). Each row: purpose, example value, where used, required/optional. Includes copy-paste `.env` skeleton and links to `SLACK_SETUP.md`, `CAPACITOR_SETUP.md`, `GOOGLE_PLAY_LAUNCH.md`, `GOOGLE_DRIVE_SETUP.md`.
- **Platform-support audit answered** (2026-02-07) — Delivered plain-language rundown to user: Android ✅ code-ready, iOS ❌ (no `ios/` folder, no Apple assets, needs Apple Dev enrollment), PWA ✅, Offline-First ✅, Capacitor ✅, Play Store ✅ ready, App Store ❌ (13-step checklist provided).
