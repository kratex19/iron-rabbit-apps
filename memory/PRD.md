...
- **Quick Guide swipe fix, take 2** (2026-02-07) — First fix (touch-action: pan-x) only helped articles opened from AppHeader. Articles opened from *within* another Radix Dialog (Settings, Kid Mode, Pantry, Meal Planner, Calendar, Barcode Scanner, Backup) still had swipes cancelled because Radix uses `react-remove-scroll` internally, which attaches a document-level `touchmove` listener (`passive:false`) that `preventDefault()`s any touch outside its DialogContent. Our QG lives at the app root (sibling of DialogContent), so touches were being cancelled by the doc listener before React's synthetic `onTouchMove` could handle them. **Fix**: added `useEffect` in `QuickGuideModal.jsx` that attaches **native** `touchstart` / `touchmove` / `wheel` listeners directly on the scroller ref while the modal is open — each calls `stopPropagation()`. Runs before Radix's doc listener → browser handles horizontal scroll natively. Verified via Playwright: `docListenerCalled: False` after synthetic dispatch on strip. Home QG (no Radix parent) still works via the earlier `touch-action: pan-x` fix.
- **Quick Guide swipe fix** — touch-action: pan-x on strip and cards.
- **Two ways to reopen Choose Your Theme** — Header 🎨 button + Settings row.
