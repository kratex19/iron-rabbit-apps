...
- **Quick Edit renders formatted HTML (styled, no tags)** — When a note's content is HTML (produced by T✦ Tags & Formatting mode in Full Screen), the Quick Edit dialog now auto-swaps its textarea for a **rendered contentEditable div** using the shared `fs-content-editable` typography — so H1/H2/H3, bold, italic, underline, strikethrough all display styled instead of showing raw `<h1>…</h1>` tags. Plain-text notes still use the classic `TextareaAutosize`. Toolbar deliberately omitted per user request (formatting still edited only in Full Screen). Underlay + brightness sliders + BG-opacity math preserved 1:1, ref-forced text color extended to the contentEditable, and paste is forced to plain text. `handleSave` runs DOMPurify again on HTML content for defence-in-depth. Unlocked NoteModal Quick Edit content block (§2b in LOCKED_SURFACES.md) with password `2020`, sentinel comment kept in place and re-locked. Service worker cache bumped `iron-rabbit-v48` → `iron-rabbit-v49`. Files: `frontend/src/notes/NoteModal.jsx`, `frontend/public/service-worker.js`.
- **Placeholder colour matches top-bar yellow icons** — The "Start writing…" placeholder in Expanded Text (Regular + Tags & Formatting modes) now paints the same orangish yellow as the FullScreenNote top-bar icons (`#eab308` dark, `#ca8a04` light). Root cause of the previous non-effect: `index.css`'s global `textarea::placeholder { -webkit-text-fill-color: hsl(var(--muted-foreground)) }` was over-painting Tailwind's `placeholder:text-yellow-500`. Fixed with dedicated `.fs-placeholder-dark` / `.fs-placeholder-light` CSS classes that force both `color` and `-webkit-text-fill-color` with `!important` on both `::placeholder` and `:empty::before`. HTML mode kept as-is (green monospace). Files: `frontend/src/notes/ExpandedTextEditor.jsx`, `frontend/src/App.css`.
- **Home page "white strip" fix (v2 — extend `<main>`)** — v1 painted the outer app-container with a composed match colour, but the visual didn't land perfectly once the slider moved. Replaced with a much simpler approach per user's suggestion: added `min-h-screen` to `<main>` so its own brightness-tracked background (the same one the slider controls) flows all the way to the viewport bottom. This guarantees a *perfect* match at every slider value because it's literally the same background surface — no composited approximation. Files: `frontend/src/NotesApp.jsx`.
- **Uncategorized header now collapsible** — On the home page (both list and grid views), the "Uncategorized" section header was a pure display component and tapping it did nothing — the tiles inside stayed expanded. `CategoryHeader.jsx` now accepts optional `onToggle`/`isOpen` props: when provided it renders as a `<button>` with a rotating ChevronDown that folds the tiles into the header, matching the behaviour of normal named categories (`CategoryGroup`). State is owned in `NotesApp.jsx` (`uncategorizedOpen`) and persisted per-device to `localStorage["ir_uncategorized_open"]`. Grid + list branches both hide the children via a CSS `hidden` class while keeping the react-beautiful-dnd Droppable/placeholder in the DOM so drag-into-uncategorized still works. Files: `frontend/src/notes/CategoryHeader.jsx`, `frontend/src/NotesApp.jsx`.
- **Caret color follows text brightness slider (global)** — The blinking text caret in every brightness-scoped surface (Home body, Quick-Text NoteModal, Expanded FullScreenNote in all three modes) now inherits the exact colour the text-brightness slider is painting. Bright-white slider → bright-white caret, near-black slider → near-black caret. Implemented via a single additive CSS rule in `index.css` (`.ir-brightness-scope input/textarea/[contenteditable] { caret-color: currentColor; }`) — no new UI, no extra slider wiring, no changes to any locked colour-painting code. Files: `frontend/src/index.css`.
- **Caret-jump fix in Format toolbar** — Fixed a bug where typing into a Header (H1/H2/H3) or Paragraph block and then tapping the Line-break icon (⤶) would jump the cursor back to before the first line of text. Root cause: `restoreSelection()` in `FormatFloatingToolbar.jsx` unconditionally clobbered the live caret with a stale `savedRangeRef` (usually the user's initial tap position at the document start). Fix: `restoreSelection()` now short-circuits when the live selection is already inside the editable, keeping the caret exactly where the user was typing (and refreshing `savedRangeRef` to match). Only falls back to the saved range when the live selection has actually been lost. Same fix implicitly repairs P/H1/H2/H3, Link, and Line-break commands. Build stamp bumped `v40-manual-wrap` → `v41-caret-keep`; service worker `CACHE_NAME` bumped `iron-rabbit-v41` → `iron-rabbit-v42`. Files: `frontend/src/notes/FormatFloatingToolbar.jsx`, `frontend/public/service-worker.js`.

### 🎉 Shipped in previous fork (2026-02-22)
...
- **Streak Recovery Reminder** — On Sundays, if the current week has zero orders logged AND the previous week did (i.e. there's an active streak worth protecting), fires a friendly `🔥 Keep the streak alive` toast. Framing adapts to freeze inventory: if the month's freeze is still ready, it reads *"Log even one order today and you'll keep the streak alive without burning this month's freeze"*; if already spent, it reads *"You've already used this month's freeze — logging an order today keeps the streak going"*. Silent no-op on other days. Fired at most once per week (keyed by Monday-anchor). New file: `frontend/src/utils/streakRecoveryNudge.js`. Wired from `NotesApp.jsx` boot.
- **Freeze Streak Trophies** — Automatic gold badge awarded once, permanently, for finishing any past calendar year with zero freezes consumed. Guardrails: only years where the user has at least one order logged qualify (so we don't retroactively gift trophies for the app's pre-history). Newly earned trophies pop with a warm golden confetti burst + a `🏆 Freeze Streak Trophy unlocked!` toast. Trophy cabinet renders under the Freeze Inventory dots as tappable amber chips (one per earned year, tap for a mini explainer toast). Storage key: `iron_rabbit_rg_freeze_trophies_v1` = `{ "YYYY": { awarded_at: ISO } }`. Files: `frontend/src/hooks/useRestaurantStats.js`, `frontend/src/components/RestaurantsStatsWidget.jsx`.
- **Wall Street Wider (preset 172)** — Regenerated with Gemini Nano Banana (gemini-3-pro-image-preview) as a proper 16:5 landscape composition. The full **"WALL STREET CRASHES"** masthead is now completely visible with the "BILLIONS LOST AS STOCKS PLUNGE" subhead below it, flanked by the "Panic on Exchange" and "Market in Ruin" column stories and center chaos photo — matching the newer landscape presets. Manifest bumped v9 → v10, service worker `CACHE_NAME` v35 → v36 and `RUNTIME` v17 → v18 so devices refresh the asset. File: `frontend/public/header-presets/header-172.webp`.
- **Freeze Inventory Center** — New compact stats card inside the Live Stats widget: `❄️ Freezes used · YYYY  N/12` with a 12-dot capacity bar (one dot per month of the current calendar year, filled sky-blue = freeze consumed, hollow = clean month, dimmed = future months). Below it a friendly caption: "Perfect year — no freezes used yet.", "X clean months · N auto-forgiven gaps.", or "Every month this year needed a freeze — nice save.". Hook exposes new fields `freezesUsedThisYear`, `freezesGrantedThisYear`, `currentYear`. Files: `frontend/src/hooks/useRestaurantStats.js`, `frontend/src/components/RestaurantsStatsWidget.jsx`.
- **Dual-Axis Compare Overlay on Sparkline** — The Live Stats tile's Sparkline now has a third mode chip: **Both**. Renders spend ($) as an emerald filled area/line against a left y-axis, plus orders (#) as a dashed amber line against an independent right y-axis. Legend chips + tiny max-value labels (top-left in emerald, top-right in amber; 0/$0 pins in the bottom corners). Combined hit targets per month — tapping still jumps to the Orders workspace filtered by that month. Files: `frontend/src/components/RestaurantsStatsWidget.jsx`.
- **Streak Freeze (Duolingo-style)** — Weekly streak now survives one missed week per calendar month, automatically. Ledger key: `iron_rabbit_rg_freeze_ledger_v1` (`{ "YYYY-MM": true }` when consumed). Freeze is granted at the start of every calendar month and is only consumed when a gap week would have otherwise broken the streak. Peek row shows a **❄️** chip next to the flame streak chip — bright sky-blue when a freeze is ready this month, dimmed + strikethrough when already used. Tap the chip for an explainer toast. Files: `frontend/src/hooks/useRestaurantStats.js`, `frontend/src/components/RestaurantsStatsPeek.jsx`.
- **Wall Mode / Kiosk toggle** — New toggle at the bottom of the Restaurants Galore Command Center: `Wall Mode (Kiosk)`. When ON, the next app cold-start auto-launches the Photo Journal fullscreen slideshow — perfect for countertop tablets. Guarded by `sessionStorage.iron_rabbit_rg_kiosk_fired_v1` so it only fires once per cold-start (not on hot reloads/focus events). Reads from `localStorage.iron_rabbit_rg_kiosk_v1`. If Photo Journal has no photos yet, a helpful toast explains. Files: `frontend/src/notes/RestaurantsGaloreDashboardModal.jsx`, `frontend/src/NotesApp.jsx` (imports `KIOSK_MODE_KEY`, adds cold-start hook + top-level `PhotoLightbox`).

### 🎉 Shipped in previous fork (2026-02, unified background picker on Home + Dashboard)
...
- **Slack Reply-to-Ack** (2026-02-07) — Drop-alert Slack messages upgraded to Block Kit with two interactive buttons: `Dismiss alert` (primary) and `Snooze 7d`, plus an `Open dashboard` URL button (when `PUBLIC_APP_URL` is set). Both buttons carry `week_start` in their `value`. New `POST /api/slack/interactive` handler verifies Slack's HMAC-SHA256 signature (5-minute replay window) via `SLACK_SIGNING_SECRET`, parses the form-urlencoded payload, and PATCHes the alert with `dismissed_via: "slack"` / `snoozed_via: "slack"` markers so the audit trail records the source. Endpoint returns 503 when the secret is unset (fail-loud on misconfig) and 401 on bad signature. Response uses `replace_original:true` so the Slack message swaps in a "Dismissed alert for week of Y-M-D" confirmation. Verified: 503 → 401 → 200 flow works end-to-end.
- **Auto-Dismiss on Recovery** (2026-02-07) — `_detect_and_ping_drop` now also sweeps active alerts and auto-dismisses any whose most-recent week's magic-link share has recovered to ≥65%. Sets `dismissed_at`, `auto_dismissed_at`, `auto_dismissed_reason: "share_recovered"`, and `auto_dismissed_at_share` (the numeric share that triggered the sweep). Audit trail preserves everything. The dashboard renders these rows with a distinctive green `AUTO-DISMISSED` badge and "recovered to X%" context line. Verified: seeded a 90% → 30% → 75% 3-window pattern, first run alerted (60pt drop), second run auto-dismissed with `auto_dismissed_at_share: 0.75`.
- **Recovery Alerts Log Panel** (2026-02-07) — Backend `GET /api/admin/recovery/alerts` + `RecoveryAlertsLog.jsx` compact strip.
- **Screenshot Freshness Strip** (2026-02-07) — Small strip on `/admin/community` showing last-regen timestamp + "Regen now" button.
- **Custom Payload Preview** (2026-02-07) — New public admin endpoint `POST /api/community/thank-you/preview` accepts optional JSON `{heading, body, nickname, contributor_email}` and renders the thank-you email HTML with those values. No DB touch, no send. Missing fields fall back to friendly demo defaults (nickname → "veggie_wizard"). The existing per-tip preview also accepts an `overrides` JSON body for A/B copy comparisons. Admin dashboard gets a new "Custom preview" toolbar button that opens a 3-field dialog (heading, body, optional nickname) and reuses the sandboxed-iframe modal for the result. Testids: `admin-custom-preview`, `custom-preview-dialog`, `custom-preview-heading|body|nickname`, `custom-preview-render`.
- **Drop-Alert Slack Ping** (2026-02-07) — Server-side detector `_detect_and_ping_drop` in `routes/analytics.py` mirrors the client-side banner: flags any adjacent-week magic_link_share drop > 20 pts (both weeks non-zero). When triggered, POSTs a formatted Slack message (":warning: Iron Rabbit — recovery funnel alert" + before/after percentages + hint) to the configured `SLACK_WEBHOOK_URL`. Idempotent: keyed on `to.week_start` in the new `recovery_alerts` collection so the same drop can't spam. New env var `SLACK_WEBHOOK_URL` (empty → still records the marker but skips the POST). Admin endpoint `POST /api/admin/recovery/check-drop` force-runs the detector. New APScheduler cron `weekly_drop_alert` fires **Monday 08:30 UTC** (30 min before the digest send). Verified: seeded a 40-pt drop scenario → first call reports `alerted:true, delta:40`, second call reports `already alerted` (idempotent).
- **Promoted-Tip Thank-You Deep Link** (2026-02-07) — `_send_thank_you` embeds an emerald "See your card on the wall" CTA linking to `${PUBLIC_APP_URL}/contributors?highlight=@<nickname>`.
- **Play Carousel Manifest** (2026-02-07) — `public/screenshots/play/framed/manifest.json` locks the 8 framed shots into a specific narrative arc: Hook (Wall Top → Wall Recent) → Engagement (Share Card) → Reassurance (Recovery → Magic Link) → Product (Home → Note Fullscreen → Quick Guide). Each entry carries position, file, caption, and story role. `python3 scripts/overlay_play_screenshots.py --list-order` prints the sequence for reference during Play Console upload.
- **Trend Threshold Banner** (2026-02-07) — `RecoveryFunnel.jsx` now surfaces an amber "Magic-link share dropped X points week-over-week" banner (data-testid `funnel-drop-alert`) when any adjacent-week share drop exceeds 20 points AND both weeks have non-zero opens. Shows the largest drop when multiple qualify. Includes week boundaries, before/after percentages, and an inline hint ("Check recent email deliverability or landing-page copy"). Verified live by seeding 2-week drop data — banner correctly rendered "40 points" alert.
- **Wall Nickname Deep Link** (2026-02-07) — `/contributors?highlight=@veggie_wizard` scrolls that card into view and pulses amber for ~2.5s.
