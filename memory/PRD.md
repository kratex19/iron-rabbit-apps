...
- **Screenshot Freshness Strip** (2026-02-07) — New `ScreenshotFreshness.jsx` mounted on `/admin/community` below the Recovery Funnel. Shows last-regen timestamp (formatted relatively — "5 min ago"), last status (ok/error/crashed/running with color-coded icon + framed_count), the trigger source (manual/cron), and a "Regen now" button. Polls `/api/admin/screenshots/status` every 3 s while a run is active; button becomes disabled + spinner during. Silently hides itself when the endpoint 404s. Testids: `screenshot-freshness-strip`, `screenshot-regen-now`, `screenshot-freshness-error`.
