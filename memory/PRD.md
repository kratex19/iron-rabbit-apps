...
- **Recovery Alerts Log Panel** (2026-02-07) — New backend `GET /api/admin/recovery/alerts?limit=N` returns the last N rows from `recovery_alerts` (newest first) with ISO-coerced timestamps. New frontend `RecoveryAlertsLog.jsx` renders a compact "Past alerts" strip under the Recovery Funnel showing delta / week / before→after pcts / relative alerted-at / Slack-vs-dashboard status. Silent (renders nothing) when history is empty — no zero-state noise on a healthy funnel. Testids: `recovery-alerts-log`, `alerts-log-row-N`, `alerts-log-refresh`.
