...
- **Promoted-Tip Thank-You Deep Link** (2026-02-07) — `_send_thank_you` in `routes/community.py` now embeds a "See your card on the wall" emerald CTA linking to `${PUBLIC_APP_URL}/contributors?highlight=@<nickname>` (URL-encoded) whenever both a nickname and PUBLIC_APP_URL are present. Anonymous tips omit the CTA. 3 new unit tests in `tests/test_thank_you_deep_link.py` verify: link present with nickname+URL, absent for anonymous tips, absent when PUBLIC_APP_URL is empty.
